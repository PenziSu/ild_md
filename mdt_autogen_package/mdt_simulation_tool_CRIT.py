...
def run_autogen_mdt_simulation(patient_json_data: dict, patient_data_summary: str, dynamic_questions_str: str) -> str:
    """
    Runs a three-phase (Plan, Discuss, Summarize) AutoGen MDT simulation.
    The summarization is guided by dynamic questions for per-patient evaluation.
    Returns the final summary from the Chair agent, intended to be a JSON string.
    """
    ...
    for cfg in specialist_configs:
        agent_llm_config = {**config.llm_config_autogen}
        function_map = {}
        if config.ENABLE_TOOLS:
            agent_llm_config["tools"] = [{"type": "function", "function": {
                "name": "query_knowledge_base_autogen_tool", "description": "Query the medical knowledge base for ILD guidelines.",
                "parameters": {"type": "object", "properties": {"query": {"type": "string", "description": "The medical query"}},"required": ["query"]}}}]
        ...
        agent = AssistantAgent(
            ...
            max_consecutive_auto_reply=cfg.get('max_consecutive_auto_reply', 3),
            function_map=function_map
        )
        agent_objects[cfg['agent_name']] = agent
        
    # Create the chair agent separately
    chair_llm_config = {**config.llm_config_autogen}
    chair_function_map = {}
    if config.ENABLE_TOOLS:
        chair_llm_config["tools"] = [{"type": "function", "function": {
            "name": "query_knowledge_base_autogen_tool", "description": "Query the medical knowledge base for ILD guidelines.",
            "parameters": {"type": "object", "properties": {"query": {"type": "string", "description": "The medical query"}},"required": ["query"]}}}]
        chair_function_map["query_knowledge_base_autogen_tool"] = query_knowledge_base_autogen_tool

    chair_agent = AssistantAgent(
        name=chair_config['agent_name'],
        system_message=chair_config['prompt'],
        llm_config=chair_llm_config,
        max_consecutive_auto_reply=chair_config.get('max_consecutive_auto_reply', 5),
        function_map=chair_function_map
    )
    ...
    try:
        ...
        traceback.print_exc()
        return f'{{"error": "Error occurred during MDT discussion: {e}"}}'
    
    discussion_history = discussion_groupchat.messages

    # --- Phase 3: Summarization ---
    print("\n--- Phase 3: Generating Final Summary ---")
    final_summary_json_str = ""
    try:
        # The Chair's system message already instructs it to output JSON.
        # We now provide a final user message with the specific dynamic questions.
        final_user_message = f"""Based on the entire discussion history, provide your final synthesis.
Your final output MUST be a single JSON object, where the keys are the questions and the values are your answers.

The specific questions to answer are:
{dynamic_questions_str}
"""
        # Make the final call to the chair agent.
        final_summary_json_str = chair_agent.generate_reply(
            messages=discussion_history + [{"role": "user", "content": final_user_message}]
        )
        print("Final summary JSON string generated successfully.")
    except Exception as e:
        print(f"Error during summarization phase: {e}")
        traceback.print_exc()
        final_summary_json_str = f'{{"error": "Error during summarization: {e}"}}'

    print(f"\n--- AutoGen MDT Simulation Concluded for Patient ID: {patient_id} ---")
    return final_summary_json_str